...
function applyVisionImpairedProfile(enable) {
  if (enable) {
    // Show color blindness options when vision profile is enabled
    toggleColorBlindnessOptions(true);

    // Apply default color blind filter
    ...
    // Hide color blindness options when vision profile is disabled
    toggleColorBlindnessOptions(false);
...
// Add event listeners for color blindness buttons
document.addEventListener("DOMContentLoaded", () => {
  const colorBlindButtons = widget.querySelectorAll(".cb-sim-btn");
  colorBlindButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      // Update active state
      colorBlindButtons.forEach((btn) => btn.classList.remove("active"));
      button.classList.add("active");
    });
  });
});

// Remove duplicate handleGesture function and keep only one version
function handleGesture(e) {
...
// Remove duplicate updateGuidePosition function and keep only one version
function updateGuidePosition(e) {
...
// Add toggle functionality for color blindness options
function toggleColorBlindnessOptions(show) {
  const colorBlindOptions = widget.querySelector("#color-blindness-options");
  if (colorBlindOptions) {
    colorBlindOptions.style.display = show ? "block" : "none";
  }
}

// Modify the vision profile toggle handler
function applyVisionImpairedProfile(enable) {
  if (enable) {
    // Show color blindness options when vision profile is enabled
    toggleColorBlindnessOptions(true);
